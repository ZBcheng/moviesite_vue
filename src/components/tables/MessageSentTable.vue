            ...
            <a-list-item slot="renderItem" slot-scope="message" :style="{ width: '90%', marginLeft: '5%'}">
                <a-list-item-meta>
                    <a slot="title" style="float:left; width: 100%; text-align: left; line-height: 30px;"
                        ...
                        <div v-if="message.receiver[0] != undefined">
                            {{ message.message_title }} (to: {{ message.receiver[0].username }})
                        </div>
                        <div v-else>
                            {{ message.message_title }} (to: 被删除的用户)
                        </div>
                    </a>
...
<script>
    ...
    export default {
        ...
        methods: {
            getMessages() {
                var url = this.host + "messages/sent?username=" + sessionStorage.username
                axios.get(url)
                    .then(response => {
            ...
            viewMessage(message_id) {
                var url = this.host + "messages/query?message_id=" + message_id
                axios.get(url)
                    .then(response => {
